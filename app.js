...
function requireOwner(){
  const u = getSession();
  return u && u.username === OWNER.username;
}
...
function wireEvents(){
  ...
  el("loginForm").addEventListener("submit", (e)=>{
    e.preventDefault();
    const res = login(normalizeDigits(el("loginUsername").value), normalizeDigits(el("loginPassword").value));
    if (!res.ok){
      toast(res.error);
      return;
    }
...
function handleImport(e){
  ...
  reader.onload = function(evt){
    ...
    rows.forEach(r=>{
      const acc = accounts.find(a => a.name.trim() === String(r["الحساب"]||"").trim());
      if(!acc) return;

      const type = r["النوع"] === "قبض" ? "in" : "out";
