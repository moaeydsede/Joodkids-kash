...
(() => {
  ...
  const SESSION_KEY = "wallet_vip_session_" + BUILD;
  ...
  const state = {
    wallets: [], walletsById: {}, walletIdByName: {}, txns: [],
    filters: { walletId: "", from: "", to: "" },
    inFlight: false, autoTimer: null,
  };
  ...
  function normalizeDateKey(raw){
    ...
    if(sl){
      let a=parseInt(sl[1],10), b=parseInt(sl[2],10), y=parseInt(sl[3],10);
      let d=a, mo=b; // dd/mm
  ...
  function renderBalances(){
    ...
    if(!state.wallets.length){
      grid.innerHTML=`<div class="muted" style="padding:14px;">لا توجد حسابات</div>`;
      return;
    }
    grid.innerHTML="";
      ...
      div.innerHTML=`
        <div class="cardHead">
          <div>
            ...
            <div class="cardType">${escapeHtml(w.type||"")}</div>
          </div>
          <div class="pill">ID: ${escapeHtml(w.id)}</div>
        </div>
        <div class="cardNums">
          <div class="numBox"><div class="l">المقبوضات</div><div class="v">${fmt(w.receipts)}</div></div>
          <div class="numBox"><div class="l">المدفوعات</div><div class="v">${fmt(w.payments)}</div></div>
        </div>
        <div class="bigBox"><div class="l">الرصيد</div><div class="v">${fmt(w.balance)}</div></div>
      `;
      div.onclick=()=>{
        state.filters.walletId=w.id;
        $("walletPick").value=w.id;
        if(!state.filters.from || !state.filters.to){
          const t=todayKey(); state.filters.from=t; state.filters.to=t;
          $("fromPick").value=t; $("toPick").value=t;
        }
        goTab("txns");
        renderTxns();
      };
  ...
  function fillWalletSelect(){
    ...
    sel.innerHTML=state.wallets.map(w=>`<option value="${escapeHtml(w.id)}">${escapeHtml(w.name)}</option>`).join("");
    if(!state.filters.walletId) state.filters.walletId=state.wallets[0]?.id||"";
    sel.value=state.filters.walletId||"";
  }

  function balanceTo(walletId,toKey){
    const opening=toNumber(state.walletsById[walletId]?.opening);
  ...
  function renderTxns(){
    ...
    const bal=balanceTo(walletId,to);
    ...
    const body=$("txBody");
    if(!rows.length){
      body.innerHTML=`<tr><td colspan="6" class="muted">لا توجد عمليات ضمن هذه الفترة</td></tr>`;
      return;
    }
    body.innerHTML="";
    rows.forEach(r=>{
  ...
  async function refreshAll(reason=""){
    ...
    try{
      ...
      state.walletsById={}; state.walletIdByName={};
      wallets.forEach(w=>{ state.walletsById[w.id]=w; state.walletIdByName[String(w.name||"").trim()]=w.id; });

      const tRows=await fetchCSV(LINKS.TXNS);
      const txns=[];
      for(let i=1;i<tRows.length;i++){
        const r=tRows[i]||[]; if(!r.length) continue;
  ...
  function login(){
    ...
    const msg=$("loginMsg");
    if(!ok){ msg.textContent="اسم المستخدم أو كلمة المرور غير صحيحة"; msg.style.color="var(--danger)"; return; }
    msg.textContent="";
    localStorage.setItem(SESSION_KEY,"1");
    showApp(true);
    const t=todayKey();
    state.filters.from=t; state.filters.to=t;
    $("fromPick").value=t; $("toPick").value=t;
    refreshAll("تحميل");
    startAuto();
  }
  ...
  function bindUI(){
    qsa(".tab").forEach(b=>b.addEventListener("click",()=>goTab(b.getAttribute("data-tab"))));
    $("btnLogin").addEventListener("click",login);
    $("btnLogout").addEventListener("click",logout);
    $("btnRefresh").addEventListener("click",()=>refreshAll("يدوي"));
    $("p").addEventListener("keydown",(e)=>{ if(e.key==="Enter") login(); });

    $("walletPick").addEventListener("change",(e)=>{ state.filters.walletId=e.target.value||""; renderTxns(); });
    $("fromPick").addEventListener("change",(e)=>{ state.filters.from=e.target.value||""; renderTxns(); });
    $("toPick").addEventListener("change",(e)=>{ state.filters.to=e.target.value||""; renderTxns(); });

    qsa("[data-range]").forEach(b=>b.addEventListener("click",()=>{
      const r=b.getAttribute("data-range"); const t=todayKey();
      ...
      $("fromPick").value=state.filters.from; $("toPick").value=state.filters.to;
      renderTxns();
    }));
  }
