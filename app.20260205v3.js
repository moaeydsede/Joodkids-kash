// app.js (no modules) — FINAL FIXED for GitHub Pages + Apps Script
(() => {
  ...
  // ✅ CORS FIX: send POST as application/x-www-form-urlencoded (no JSON headers)
  async function apiPost(body) {
    if (!API_BASE) throw new Error("API_BASE غير مضبوط داخل config.js");
  ...
  function isUserTyping(){
    ...
    if (!a) return false;
    const id = a.id || "";
    return ["u","p","q","fromPick","toPick","walletPick","mDate","mWallet","mAccount","mRec","mPay","mDesc","mRef"].includes(id);
  }
  ...
  function fillSelects(){
    ...
    sel.innerHTML = `<option value="ALL">كل الحسابات</option>` + state.wallets.map(w => `<option value="${w.id}">${w.name}</option>`).join("");
  ...
  async function refreshAll(reason=""){
    ...
    try{
      ...
      renderKpis({ grand, todayKey: init.todayKey || "" });
      fillSelects();
      renderBalances();
  ...
  function openModal(yes){
    $("modal").style.display = yes ? "flex" : "none";
  }
